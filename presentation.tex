  ...
  \begin{columns}
    \begin{column}{.5\textwidth}
      \begin{tikzpicture}[nodes={shape=circle,draw,fill=GenericItem}]
        \node (core) [alt=<6>{fill=GenericHighlight}{}] {Uberon};
        \node (fbbt) [alt=<{1,7}>{fill=GenericHighlight}{}] at (150:2cm) {FBbt};
        \node (zfa)  [alt=<{2,7}>{fill=GenericHighlight}{}] at (90:2cm)  {ZFA};
        \node (xao)  [alt=<{3,7}>{fill=GenericHighlight}{}] at (30:2cm)  {XAO};
        \node (ma)   [alt=<{4,7}>{fill=GenericHighlight}{}] at (210:2cm) {MA};
        \node (wbbt) [alt=<{5,7}>{fill=GenericHighlight}{}] at (270:2cm) {WBbt};
        \node (tx1) at  (310:2cm) {...};
        \node (tx2) at  (330:2cm) {...};
        \node (tx3) at  (350:2cm) {...};
    ...
    \begin{column}{.45\textwidth}
      ...
      \begin{overlayarea}{\textwidth}{1cm}\color{red}
        ...
        \only<2>{Zebrafish Anatomy Ontology}
        \only<3>{Xenopus Anatomy Ontology}
        \only<4>{Mouse Anatomy Ontology}
        \only<5>{\emph{C. elegans} Anatomy Ontology}
      \end{overlayarea}
...
\begin{frame}
  ...
  \begin{overlayarea}{\textwidth}{2.5cm}
    \only<1-2>{\textcolor<1>{red}{Select mappings with a UBERON subject}

    and \textcolor<2>{red}{flip them around the predicate}}
    \only<3-4>{\textcolor<3>{red}{Select mappings with a \emph{skos:broadMatch} predicate}

    and \textcolor<4>{red}{create a \emph{SubClassOf} axiom}}
    \only<5-10>{Select mappings
      \begin{itemize}
